                     ...
                     if getattr(ax, 'name', '') == '3d':
                         ...
                         if h > 0:
                             ...
                             try: 
                                 ...
                                 # As the stretched box rotates, its long corners swing forward and clip.
                                 # We dynamically zoom the camera out based on the current rotation angle.
                                 azim_rad = np.radians(ax.azim)
                                 elev_rad = np.radians(ax.elev)
                                 
                                 # Calculate corner protrusion using trigonometry
                                 h_stretch = abs(np.cos(azim_rad)) + abs(np.sin(azim_rad))
                                 v_stretch = abs(np.cos(elev_rad)) + abs(np.sin(elev_rad))
                                 
                                 # Apply a baseline zoom (7.5) and scale it by the maximum stretch
                                 # This causes the plot to gracefully "breathe" out when corners approach the screen edge
                                 ax.dist = 7.5 * max(h_stretch, v_stretch)
                             except: pass
